...
function toastTopFactory() {
  ...
  if (!toastEl) {
    ...
    toastEl.style.cssText =
      ...
      "background:#0f172a;color:#ffffff;padding:.6rem .9rem;" +
      "border-radius:10px;box-shadow:0 8px 22px rgba(2,6,23,.08);" +
      "z-index:9999;font-weight:700;display:none";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Update the verified + donor tier badges
  function updateBadges(data) {
    ...
    const vIcon = verifyBadge.querySelector(".icon");
    if (isVerified) {
    ...
    const tierLabel =
      data.donorTier || computeDonorTier(currentDonations);
    tierText.textContent = tierLabel;

    const tIcon = tierBadge.querySelector(".icon");
    if (tIcon) {
      const lower = tierLabel.toLowerCase();
      if (lower.includes("gold")) tIcon.textContent = "🥇";
      else if (lower.includes("silver")) tIcon.textContent = "🥈";
      else if (lower.includes("bronze")) tIcon.textContent = "🥉";
      ...
      async (ss) => {
        ...
        try {
          ...
          if (donationsCount !== lastCount) {
            ...
            if (newTier !== prevTier) {
              const tierMsg = `Your donor tier is now ${newTier}`;
              pushActivity(tierMsg, nowStr());
              toastTop(tierMsg);
            }
      ...
      <article class="request-card" data-id="${escapeHtml(
        ...
        <div style="flex:1;min-width:0">
          <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;">
            <div style="min-width:0">
              <div class="item-title" style="font-weight:800;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escapeHtml(
                r.title || "Request"
              )}</div>
              <div class="item-sub muted" style="font-size:.95rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escapeHtml(
                r.description || r.subtitle || ""
              )}</div>
            </div>
            <div style="text-align:right;min-width:90px;">
              <div class="item-date muted" style="font-size:.85rem">${escapeHtml(
                r._when || ""
              )}</div>
            </div>
          </div>
        ...
        if (requestsList && !requestsList.__bound) {
          ...
          requestsList.addEventListener("click", async (e) => {
            ...
            if (openBtn) {
              ...
              location.href = `request.html?rid=${encodeURIComponent(
                id
              )}&view=mine`;
              return;
            }
      ...
      unsubUserDoc = onSnapshot(
        ...
        async (snap) => {
          ...
          const userData = {
            ...
            donorTier: data.donorTier || computeDonorTier(data.donations || 0),
          };
